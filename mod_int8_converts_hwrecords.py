...
def command_maker(cfg, module, num):
    data_loader_script = f"./data-loaders/{cfg}-{module}.py"

    return [
            "polygraphy",
            "convert",
            f"generated/onnx/{args.cfg}-{args.model_name}-{num}.onnx",
            "--int8",
            "--data-loader-script",
            data_loader_script,
            "--output",
            f"generated/trt/int8-{args.cfg}-{args.model_name}-{num}.engine"
    ]
...
def write_to_file(model, elapsed, cpu_data, gpu_data, mem_data):
    ...
    with open(f'./results/tensorrt-compress-details/int8-{model}.txt', 'w') as file:
...
def main(args):
    ...
    # Warmup
    command = command_maker(args.cfg, args.module, 1)
    ...
    for i in range(1, 2):
        ...
        # Start subprocess
        command = command_maker(args.cfg, args.module, i)
